...
use \think\Request;
use \think\View;
class Mori extends Common{
    public function index(){
        $id = Request::instance()->get('id',0);
        if ($id==0){$id=1;}
        $mori = new MoriM();
        $list = $mori->get_moriList(array('ParentId'=>$id));
        $temege = $mori->get_moriInfo(array('id'=>$id));
        if (empty($temege)){$temege['img'] = '';$temege['word']='';$temege['mp3']='';$temege['id'] = $id;}
        if ($temege['img']=='0'){$temege['img']='/public/img/ma.jpg';}
    ...
    public function delete(){
        ...
        if ($type==2){
            if ($id>1){
                ...
                $data = $temege_model->get_moriInfo(array('id'=>$id));
                $temege_model->delete_moriInfo(array('id'=>$id));
                /** 用上一级id作为引导 */
                $this->success('删除成功','/admin.php/admin/mori/index?id='.$data['ParentId'],'',1);
            }
        }
        $this->redirect('/admin.php/admin/mori/index?id='.$id);
    ...
    public function insert(){
        ...
        if ($tip!=0){
            ...
            if ($type==0){
                ...
                $data['ParentId'] = $id;
                $temege_model->insert_moriInfo($data);
                $this->success('添加成功','/admin.php/admin/mori/index?id='.$id,'',1);
            }else{
                $data = $temege_model->get_moriInfo(array('id'=>$id));
                $data['word'] = Request::instance()->post('word','');
                $data['Meaning'] = Request::instance()->post('Meaning','');
                ...
                $mp3 = upload_file('mp3');
                if ($mp3!='0'){$data['mp3']=$mp3;}
                $temege_model->save_moriInfo($data,array('id'=>$id));
                $this->success('修改成功','/admin.php/admin/mori/index?id='.$id,'',1);
            }
        }else{
            /** @var  $type   0 是添加*/
            $type = Request::instance()->get('type',0);
