<?php
	...
	$id = $_GET['id'];
	$sql = "SELECT * FROM honi WHERE id = " . $id;
	...
	if($arr['img'] == "")
	{
		$img = "img/shanyang.jpg";
	}else
	{
		$img = 'file/'.$arr['img'];
	}
	$sql = "SELECT * FROM honi WHERE ParentId = ".$id;
	startDB();
	$q = execute_sql_select($sql);
	...
	<div class="main">
        ...
        <table class="list-tb" border="0" cellspacing="0" cellpadding="0">
          ...
          {
          	echo '<td valign="top"><span><a href="honi.php?id='.$row['id'].'">'.$row['word'].'</a></span></td>';
          }
